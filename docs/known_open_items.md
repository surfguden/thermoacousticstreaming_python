...
| HW-AD2-BNC-001 | REQUIRES_PHYSICAL_VALIDATION | Official adapter semantics establish W1/J4/JP4 and W2/J5/JP5 with direct or approximately 49.9-ohm series paths. Exact installed PCB revision, jumper positions, and downstream input impedances are unverified. Programmed AD2 voltage is therefore not established downstream voltage. | During a powered-down authorized inspection, retain readable PCB/jumper evidence and establish load impedances; derive or measure the loaded voltage. | Does not block a bounded run if records and claims remain requested/effective AD2 settings rather than downstream voltage or pressure. |
...
The next step is not another general software audit or work on every deferred
item. Complete the independent pre-hardware readiness review described in
`project_control.md`. Only a concrete software blocker to the narrow minimal
Internal-trigger camera + finite Acoustic/W1 run should return the project to
software implementation. All hardware remains separately authorized.
