...
| ARCH-RECORD-001 | OPEN | Experiment record lifecycle | A stopped series has no durable aggregate showing requested versus completed repeats | Per-repeat TDMS identity/outcome now distinguishes `IN_PROGRESS`/`COMPLETED`/`FAILED`, but later unstarted repeats have no record or series manifest | Decide whether to add a bounded series-level manifest after planning authority work is separately scoped | A retained series-level record distinguishes requested, started, completed, aborted, and failed repeat counts without overstating hardware application |
