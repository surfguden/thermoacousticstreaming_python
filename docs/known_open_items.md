...
| --- | --- | --- | --- | --- | --- | --- |
| HW-TIMING-001 | DEFERRED / READY FOR PHYSICAL VERIFICATION | Camera / AD2 / DIO | Physical trigger and exposure timing cannot be claimed; ordinary software development is not blocked | Owner photographs identify the camera as ORCA-Fusion BT `C15440-20UP`, S/N `500478`, and confirm DIO0/pink at its `EXT.TRIG` input, distinct from `TIMING 1/2/3` outputs. Hamamatsu documents `EXT.TRIG` as TTL/3.3-V LVCMOS, 10-kohm input with selectable polarity. Normal production configures neither DIO0 nor DIO1 and retains DCAM `Internal`; the earlier read-only probe found all three timing outputs fixed `LOW`. No acquisition, output, trigger change, or timing measurement occurred | Classify DIO0 as `CONNECTED_BUT_CURRENTLY_UNUSED` for standard steady/quasi-steady work. Reopen only under separately authorized synchronized/transient verification with explicit external-trigger mode/polarity and a bounded timing-observation plan; keep DIO1 unprogrammed pending laser semantics | Retained current-hardware evidence establishes requested, electrical, and physical timing without inferring synchronization from cable presence |
...
- **Current AD2 physical wiring is owner-confirmed; electrical semantics are
  not.** Project Ch1 maps through `exp_ad2_channels[0]` and
  `WfgChannelConfig.channel_index=0` to physical W1 and the acoustic
  amplifier/transducer. Project Ch2 maps through index 1 to physical W2 and the
  laser Analog In/control BNC. DIO0/pink is connected to camera trigger;
  DIO1/green is connected to a separate laser digital-trigger input. The SDK
  index translation and production software paths are verified from source and
  Digilent documentation. No output or timing was physically verified. Normal
  production therefore fails before hardware configuration if W2 carrier/FM is
  enabled and configures neither DIO0 nor DIO1. **Status: SOFTWARE MAPPING
  VERIFIED; LASER INPUT SEMANTICS AND PHYSICAL TIMING UNVERIFIED.**
...
- **Shared runtime evidence and preflight are intentionally incomplete.**
  ...
  are deferred rather than guessed. V3 renders the shared snapshot and shadow
  `BuildResult` as presentation/audit derivation; normal Start uses the
  independent RunPlan path and the legacy builder is rollback-only. The exact
  P0 evidence procedures and persistence proposal are in
...
- **AD2 W1 acoustic output / historical DIO1 timing was
  ...
  Owner-supplied current wiring now establishes DIO0/pink as the physical
  camera-trigger cable and DIO1/green as the separate laser-trigger cable.
  Current normal experiments explicitly configure neither DIO0 nor DIO1 and
  retain DCAM trigger source `Internal`; cable presence does not establish use
  or timing. `Camera Start` remains planned sequence metadata and is
  ...
  **Later connector/output review:** the installed Digilent WaveForms manual
  ...
  query confirmed three connectors on camera `S/N: 500478`. Fresh TIMING 1
  readback is `kind=LOW`, `mode=EDGE`, `source=READOUT END`, negative polarity,
  delay `0`, and period `1 ms`. Because `kind=LOW` selects a constant low
  output, the subordinate source, mode, polarity, delay, and period are not
  active physical timing behavior. `configure_trigger_global_exposure()`
  changes the camera exposure mode, not these physical output connectors.
  Therefore no camera exposure edge is presently available without a deliberate
  temporary output-property write and subsequent restoration. No such write,
  acquisition, or AD2 output was issued. **Status: DEFERRED / READY FOR
  PHYSICAL VERIFICATION.** This does not block ordinary software development,
  but it blocks any claim that AD2, DIO1, and camera exposure are physically
  synchronized.
- **Z-scan previously bypassed the configured Thorlabs device identity.** The
  ...
  `_start_zscan()` handlers previously each constructed a fresh
  `PiezoStage()` using its class default serial. Commit `42fee3b`/`df50083`
  now routes range, target validation, motion, and cleanup through the
  initialized `Application.z_motor.stage`; no independent discovery or
  disconnect remains in Z-Scan. No hardware was accessed and no physical
  stage identity was verified by this software correction. **Status: RESOLVED
  FOR SOFTWARE OWNERSHIP; HARDWARE_EVIDENCE_REQUIRED for real Z-scan use.**
...
- **pytest scratch-directory accumulation and Windows ACL lock remain
  unresolved.** F0 (2026-09-01) removed the repository-relative default
  `--basetemp`: a fresh create/write/read/rename/delete probe directly under
  `%TEMP%` succeeds, and a normal pytest invocation now creates no new
  repository-root scratch. This does not contradict the historical failure:
  pytest previously failed while creating/scanning its deeper
  `%LOCALAPPDATA%\\Temp\\pytest-of-<user>` hierarchy, while the old root
  directories themselves remain ACL-protected and reject deletion. Session 49 found 119 `.pytest_tmp_*` leftovers, 35 of them
