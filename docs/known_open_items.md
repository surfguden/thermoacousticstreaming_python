...
| SW-HANDOVER-SEQUENCE-001 | OWNER_DECISION_REQUIRED | Current runtime is sequential: it waits for the programmed AD2 completion window, flushes before normal TIFF/TDMS image saving, and persists `FlushCompleted` in TDMS. The handover's save/flush concurrency and earlier completion boundary are intended design, not implemented behavior. W1/W2 output-window overlap with flush/save is not owner-approved physical behavior. | Adjudicate output-completion safety policy, worker/rendezvous/failure precedence, Qt/thread context, and single-writer TDMS ownership before implementation. A design candidate is hardware-only flush worker -> structured result -> canonical finalizer persists flush evidence. | None while current runtime remains unchanged. |
