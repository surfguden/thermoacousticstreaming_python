...
| Experiment — 1 Preparation checklist | Operator preparation prompts for equipment readiness, environment/temperature, sample/fluidics, pump preparation, imaging/focus, laser/optics, and acoustic precheck. **Local presentation state only:** its confirmations are not persisted run evidence and are never `PHYSICAL_VERIFIED`. Opening it performs no hardware I/O. The two rows whose own text points at a specific Manual & Service panel (Guided Pump Preparation → Pump & Valve; Imaging / Focus → Camera) carry a button that opens that panel directly, using the same `_open_manual_panel()` the Manual & Service workspace itself uses. Navigation only; the panel's actions keep their own established gates regardless of how it was opened. |
