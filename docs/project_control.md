...
**Checkpoint P2 (Configure / Review, 2026-09-06)** reviewed Configure's
information architecture and progressive-disclosure classification against
the target product model (Run/Acquisition/Conditions/Acoustic/Refresh/
Advanced) and found it already substantially matches: Acquisition, Conditions
(the temperature program), Acoustic/W1, Repeat Sample Refresh, and Advanced
WFG are already separate tabs in that order, and fields already reviewed as
engineering/API (deferred camera metadata, Advanced WFG's raw channel
controls) or diagnostic/retained (the Camera panel's "Retained (not used by
runtime)" group) were already correctly isolated with an explicit note. The
one genuine coverage gap found was Review: Section 9.3-style projection
listed refresh but not whether Prepare's own pump preparation actually left
the pump ready for it — closed by the "Pump / fluidics readiness" row above.
...
| Experiment — 3 Review run | Full-width canonical-request-derived run scope, sequence, camera, Acoustic/W1, unavailable laser control, refresh, **pump / fluidics readiness**, output, required devices, blockers, warnings, and requested/latest-applied evidence. Its DIO/completion timing rows are projected from the canonical plan, not recomputed from widgets. **Since `V3_OPERATOR_WORKFLOW_PRODUCTIZATION` P2 (2026-09-06)**: a "Pump / fluidics readiness" section reports whether Prepare's own pump preparation (reference move confirmed, syringe configured, tracked fill) actually leaves the pump ready for the automated repeat-to-repeat refresh Configure requested — `NOT REQUIRED` when refresh is off, `NOT AVAILABLE` when pump/valve are disabled (the runtime will skip the refresh), otherwise the cached state with a descriptive (non-blocking) warning if reference move or syringe configuration is still outstanding. It reuses the exact `self.app.pump`/`self.app.valve` cache Prepare's own readiness label and Manual & Service's cached-status group already read — no new hardware query, no second authority. |
...
**Hardware-session lifecycle (`CHECKPOINT_S_CLOSURE_DEBUG_AND_HARDWARE_SESSION_LIFECYCLE_V2`, 2026-09-07).** Initialize hardware now has an explicit lifecycle counterpart, "Shutdown hardware", beside it in the persistent instrument bar (not Manual & Service):

```text
NOT INITIALIZED -> Initialize hardware -> INITIALIZED -> Shutdown hardware -> NOT INITIALIZED -> Initialize hardware -> INITIALIZED
```

all inside the same V3 process. "Shutdown hardware" is deliberately not called "Disconnect" or "Power off hardware": it reuses the exact canonical `Application.cleanup()` authority Application Exit already used (via the inherited, unmodified `MainWindow._start_shutdown()` coordinator, now called with `close_after=False`) -- the same call that stops Pump motion (`CetoniPump.cleanup()` -> `stop()` -> `backend.close()`, the identical backend authority manual Stop pump reaches), disables TEC static output, stops/resets AD2 AnalogOut and DigitalOut, and stops/releases the Camera and Z-stage. No second cleanup implementation and no second Pump-stop implementation were introduced. It is gated by the same `_busy_count`/`_experiment_series_active`/`_zscan_active`/`_manual_z_operation_active` flags this window already used to gate manual Z motion (Option A of the task's active-work policy: disabled while busy, not raced against a live worker), and `_start_shutdown()`'s own pre-existing `_shutdown_in_progress` guard makes a double click and a rapid double-shutdown safe. Application Exit's own pre-existing behavior (no active-work gate before `_start_shutdown(close_after=True)`) was independently reviewed and found unchanged by this checkpoint -- deliberately not touched, since Exit must remain deterministic on its own and this is a narrower, additive button, not a redesign of Exit's cancellation boundary; see `known_open_items.md` for that as a recorded, non-blocking follow-up. Reinitialization works because `_initialize_system()` already called `Application.cleanup()` before rebuilding the hardware bundle and calling `Application.initialize()` -- Shutdown hardware only exposes that existing idempotent-cleanup step as its own explicit operator action. Operator-set experiment configuration (camera exposure/FPS, syringe selection, TEC target, etc.) lives in Qt widgets, which cleanup()/initialize() never touch, so it survives the round trip unchanged; runtime/connection state (`initialized`/`capturing`/`device_handle`) is reset by each instrument's own `cleanup()`, so the connection indicators stop claiming Connected. Tests: `tests/test_v3_hardware_session_lifecycle.py`.
