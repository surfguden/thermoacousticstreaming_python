...
| 2026-09-06 | `V3_SHAKEDOWN_USABILITY_CONSOLIDATION` | Standalone usability instruction; explicitly re-opened two `AWAITING_SHAKEDOWN` items for a second look rather than accepting the prior deferral automatically | Owner; two bounded checkpoints on `junjiebranch` | `0338e3a`, `6b5f1ad` | Executed. Split the persistent Execution strip into two rows by role, substantially reducing (not eliminating) its width shortfall -- measured before/after at every supported size. Found and fixed the actual bottleneck on Review/Timing (two non-wrapping `QGridLayout` columns): word-wrapping them alone dropped its minimum width from 1104 px to 552 px, fully closing that page. Applied the same technique to two V3-only Configure/Acquisition labels for a partial reduction (1020 px to 924 px, 8 px short of the 980 px floor's viewport); identified Configure/Acquisition's residual driver as a dense spinbox grid and Repeat Sample Refresh's as `_experiment_flush_group()`, confirmed shared with V1, and explicitly deferred both rather than pursuing a layout redesign or a cross-surface edit. Tried and reverted a word-wrap change that measured zero effect, rather than keeping it as unexplained diff. Separately closed `SW-TRACE-CONCURRENCY-TEST-001` by moving its already-specified assertions into the concurrent-overlap test; run 9 times with zero flakes. No hardware semantics, trigger architecture, canonical execution authority, or fail-closed gate changed; one `TEST-QT-LIFETIME-001`-family flake observed in a full-suite run (a new node id, same symptom, passed 4/4 in isolation), not treated as a new regression. |
| 2026-09-06 | `V3_WORKFLOW_CHRONOLOGY_RECONCILIATION` | Standalone checkpoint combining chronology reconciliation with closeout of any genuinely justified prior-round residuals | Owner; one bounded implementation checkpoint on `junjiebranch` | `bc51000` | Executed. Investigated all eight areas the instruction named (pump, camera, Z, TEC, other experiment parameters, Review/Start/Monitor, Manual Service/Diagnostics, remaining closeout items). Found one genuine defect: the Preparation checklist's row order (Equipment, **Environment/Temperature**, Sample/Fluidics, Guided Pump Preparation, Imaging/Focus, Z/Positioning, Laser/Optics, Acoustic Precheck) contradicted both its own rationale comment and the owner chronology by placing TEC second, ahead of pump/camera/Z. Reordered to Equipment → Guided Pump Preparation → Imaging/Focus → Z/Positioning → Environment/Temperature → Sample/Fluidics → Laser/Optics → Acoustic Precheck, matching Configure's own Acquisition → Conditions order (`37088bd`) instead of contradicting it. Updated the shifted `v3PrepareOpenPanel*`/`v3PrepareOpenConfigure*` object-name indices in `tests/test_v3_usability_shakedown.py` and added a non-vacuous full row-order regression assertion to `tests/test_qt_ui_v3.py`. Investigated the other seven areas and found no gap requiring a change, each confirmed from source rather than accepted from prior reports: Z-scan (`_zscan_parameters_group()`/`_zscan_control_group()`) is already distinct from single-position manual focus (`_manual_focus_group()`); the only TEC control surface is consistently labelled "TEC temperature scan" throughout (groupbox, checkbox, tab) with no separate simple-setpoint control to conflate it with; syringe is already nested two levels under Guided Pump Preparation → Pump & Valve panel → Syringe setup sub-tab, never a top-level or peer object, and its sub-tab naming was deliberately chosen and tested across two prior rounds (left untouched); Manual & Service/Diagnostics structure and Review/Start/Monitor showed no gap beyond what prior checkpoints already closed. Re-checked the six `NONBLOCKING_SOFTWARE` open items (execution-strip width, the other two narrow-window pages, trace I/O cost, the flush-duplicate logging shape, the review fluid predicate, `TIMING_MINTRIGGERINTERVAL`, `TEST-QT-LIFETIME-001`) against "closeable now with strong evidence": none qualified — each remains genuinely gated on hardware measurement, an operator-observation judgement, or a layout/logic change to `application.py`/`experiment_planning.py` outside this checkpoint's UI-only risk budget; none reopened, none closed. Full offline suite (`QT_QPA_PLATFORM=offscreen`, `-p no:cacheprovider`, external `--basetemp`) 830 passed, 1 skipped, run twice clean before and after. No trigger architecture, canonical execution authority, planner, executor, or hardware gate touched; no new hardware read added. |
| 2026-09-06 | `V3_PRODUCTIZATION_AND_SOFTWARE_PERMISSION_UNLOCK`, continued as `V3_PERMISSION_UNBLOCK_AND_PRODUCTIZATION_CONTINUATION`, then two follow-on unnamed passes (a workflow-to-UI reconciliation pass, then a closeout pass) -- four dispatches against the same `3f7221f` baseline, treated as one continuous checkpoint since each reissue asked for the same operator-chronology work with progressively more evidence (the third supplied `docs/experiment_sequence_timeline.txt` and `docs/handover_sequence_reconciliation_20260904.md` as reconciliation input) | Owner; two bounded implementation checkpoints on `junjiebranch` | `0994046`, `37088bd` | Executed. `0994046`: reordered the manual Pump & Valve panel's tabs (Syringe setup first, ahead of Pump/Valve/Flush/Recovery) so the one-time reference-move calibration precedes routine refill/empty, matching the chronology V1's own precedent test already establishes. Found the Preparation checklist's "Imaging / Focus" row named both camera and focus but its quick-open button only ever reached the Camera panel -- Z-stage motion lives in a separate panel (Z-Scan) the row never linked -- and split it into two rows, each linking only what it names. Found "Environment / Temperature" named TEC setup in prose with no direct route to it and added a Configure-tab-switch button (`_v3_open_configure_tab()`, a distinct mechanism from the panel quick-opens, looked up by object name rather than a hardcoded index). `37088bd`: found Configure's own sub-tab order placed Conditions (TEC) after Acoustic/W1, one of the "other experiment parameters" the chronology puts after TEC, not before -- swapped only that adjacent pair (Acquisition stays first; Repeat Sample Refresh and Advanced WFG keep their indices), and converted the one test that hardcoded the Ad2 tab's index to an object-name lookup so it survives future reorders. Read `experiment_sequence_timeline.txt` and the handover reconciliation record in full: both describe the automated per-repeat runtime hardware sequence (flush/pump/valve/camera-trigger/AD2 timing during Start), not an operator manual-preparation ordering, so they do not conflict with these UI changes -- they answer a different question. Investigated and explicitly left alone, with reasons recorded in the commit messages: TEC/Z/Camera manual-control panel placement (no gap found), a second advanced/command surface (Manual & Service already is one), the two remaining `AWAITING_SHAKEDOWN` narrow-window/execution-strip items (reconfirmed by fresh inspection, not just accepted), `SW-TRACE-FLUSH-DUPLICATE-001` (touches `application.py` execution-flow logging, outside this checkpoint's risk budget), and the Preparation checklist's own row order (a set of independent local confirmations, not a sequential gate, unlike the pump panel's and Configure's own tab orders). Added lessons_learned.md 7.10 (a quick-open button must reach everything its own row promises) and this row. No hardware semantics, trigger architecture, canonical execution authority, or fail-closed gate changed; no new hardware read added. Full offline suite 830 passed, 1 skipped before each commit; one `TEST-QT-LIFETIME-001`-family flake seen in an interim partial run (unrelated V1 window construction), re-run 4/4 in isolation and absent from the full clean run. |
| 2026-09-06 | `V3_OPERATOR_WORKFLOW_PRODUCTIZATION`, Checkpoint P1 | Owner; a bounded three-checkpoint (P1/P2/P3) implementation program on `junjiebranch`, P1 scoped to routine Pump/Camera/Z/TEC preparation | `75d1d1b` | Executed. Reused `bc51000`'s already-correct Prepare row order/chronology as its baseline and moved past information relocation into interaction productization: Guided Pump Preparation, Imaging / Focus, Z / Positioning, and Environment / Temperature now embed their routine controls directly (see the commit message for the full authority-reuse account). qt_ui.py's `_pump_tab()` (V1) and qt_ui_v3.py's own `_pump_tab()`/`_camera_tab()`/`_zscan_tab()` overrides (V3 Manual & Service) were decomposed into composable `_pump_*_group()`/`_v3_camera_*_group()` methods with no V1 behavior change, verified by `tools/audit_change_surface.py --symbol _pump_tab --symbol _camera_tab --symbol _zscan_tab --symbol _manual_focus_group`. TEC gained a genuinely new manual action (no prior non-scan authority existed) calling the exact `TecController.apply_static_setpoint()`/`wait_until_stable()` the automated temperature-series runner already calls. Geometry proven at 1366x768/1440x900/1920x1080 by extending the existing parametrized containment test rather than adding a parallel one. Full offline suite 835 passed, 1 skipped, 0 failed on the landing run; two earlier development runs each hit one distinct pre-existing `TEST-QT-LIFETIME-001` node id (`test_v3_dialogs_open_at_usable_sizes_without_full_path_width`'s own documented flake, and `test_camera_sequence_group_flags_live_automated_use_and_dead_capture_mode`, one of the three baseline-documented node ids), both re-confirmed passing in isolation, consistent with the documented ~2-in-10 baseline. P2 (Configure/Review) and P3 (integration/closeout) remain; per the program's Section 22, a fresh session performs the independent review after all three checkpoints are pushed. |
