...
| GlobalExposure | Requests the camera `GLOBALRESET` behavior; may require a compatible trigger source. False deliberately leaves the property unchanged. | Not the numerical exposure time and not a DIO duration. | Camera trigger/exposure behavior option with unresolved applicability. | `qt_ui_v3.py:744-748`; `application.py:735-740` |
...
7. **Temperature scan and manual panels** — leave separate. Temperature is a
   ...
   (`qt_ui_v3.py:1004-1025`). **[PROJECT FACT + JUDGMENT]** Combining these surfaces
