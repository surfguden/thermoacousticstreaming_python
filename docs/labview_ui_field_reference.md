...
| ConfigureSyringe button | "Configure" | `configure_syringe()` exists and remains gated behind this same explicit manual/service action; selecting a dropdown value alone does not apply geometry to hardware | match; current pump readiness remains in `project_control.md` / `known_open_items.md` rather than this migration table |
