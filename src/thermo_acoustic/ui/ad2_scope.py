...
class OscilloscopePanel(QWidget):
    ...
    def __init__(self, device_panel, parent=None) -> None:
        ...
        self.sample_rate = self._profile(
            "scope_sample_frequency_hz", _double_spin(10_000, 0.001, 100_000_000, 3)
        )
        ...
        self.poll_interval = self._profile(
            "scope_poll_interval_s", _double_spin(0.01, 0.001, 60, 3)
        )
        ...
        for label, widget in (
            ...
            ("Poll interval (s)", self.poll_interval),
        ...
        self.channel_range: list[QDoubleSpinBox] = []
        ...
        for index in range(2):
            ...
            range_v = self._profile(
                f"{prefix}_range_v", _double_spin(5, 0.001, 50, 3)
            )
        ...
        self.trigger_type = self._enum_combo("scope_trigger_type", Ad2ScopeTriggerType)
        ...
        self.trigger_hysteresis = self._profile(
            "scope_trigger_hysteresis_v", _double_spin(0.01, 0, 50, 4)
        )
        self.trigger_length_condition = self._enum_combo(
            "scope_trigger_length_condition", Ad2ScopeTriggerLengthCondition
        )
        self.trigger_length = self._profile(
            "scope_trigger_length_s", _double_spin(0, 0, 3600, 6)
        )
        ...
        detector_fields = (
            ...
            ("Length condition", self.trigger_length_condition),
            ("Length (s)", self.trigger_length),
    ...
    def arguments(self) -> Ad2ConfigureScopeArgs:
        channels = tuple(
            Ad2ScopeChannelArgs(index, self.channel_range[index].value(), self.channel_offset[index].value())
            for index in range(2)
        ...
        trigger = Ad2ScopeTriggerArgs(
            ...
            trigger_type=Ad2ScopeTriggerType(self.trigger_type.currentData()),
            ...
            length_condition=Ad2ScopeTriggerLengthCondition(
                self.trigger_length_condition.currentData()
            ),
            length_s=self.trigger_length.value(),
        ...
        return Ad2ConfigureScopeArgs(
            ...
            poll_interval_s=self.poll_interval.value(),
            trigger=trigger,
        )
    ...
    def validate_profile_values(values: dict[str, object]) -> None:
        ...
        if float(values["scope_poll_interval_s"]) > float(values["scope_timeout_s"]):
            raise ValueError("scope_poll_interval_s must not exceed scope_timeout_s")
    ...
    def handle_result(self, result: object) -> bool:
        ...
        for index in range(2):
            ...
            if channel is not None:
                self.channel_range[index].setValue(channel.range_v)
