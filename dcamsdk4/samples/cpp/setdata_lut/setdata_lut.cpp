...
BOOL dcamcon_set_lutparam( HDCAM hdcam, int32 nPage )
{
	...
	if( luttype & DCAMDATA_LUTTYPE__SEGMENTED_LINEAR )
	{
		...
		double bitsperchannel = 16;
		err = dcamprop_getvalue(hdcam, DCAM_IDPROP_BITSPERCHANNEL, &bitsperchannel);
		if (failed(err))
		{
			dcamcon_show_dcamerr(hdcam, err, "dcamprop_getvalue()", "DCAM_IDPROP_BITSPERCHANNEL");
			return FALSE;
		}

		int32 inMax = (1 << (int32)bitsperchannel) - 1;
		...
		set_linearpoint( pLinearlut, nSetPoint, inMax, 4095 );
